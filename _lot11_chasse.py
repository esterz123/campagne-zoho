#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Lot11 : chasse domaines + analyse HTML (parallele). Usage: python _lot11_chasse.py"""
...
if __name__ == "__main__":
    out = {}
    try:
        out = json.load(open(BASE + r"\_lot11_domaines_tmp.json", encoding="utf-8"))
    except Exception:
        out = {}
    for i in range(110, 150):
        ...
        f = r.get("trouve")
        if f:
            print(ii, "|", r["nom"][:32], "->", f["url"].replace("https://", "").replace("http://", ""),
                  "|", f["titre"][:45], "| ©", f["copyright"], f["cms"], f["tech"], "|", f["emails"][:2], flush=True)
        else:
            print(ii, "|", r["nom"][:32], "-> RIEN", flush=True)
        json.dump(out, open(BASE + r"\_lot11_domaines_tmp.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
    print("TERMINE 110-149", flush=True)
